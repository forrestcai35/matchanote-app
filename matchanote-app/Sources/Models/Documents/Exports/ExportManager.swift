...
class ExportManager {
    ...
    func exportNoteAsImage(_ note: Note, selectedPages: [Int]? = nil) -> [URL]? {
        ...
        for page in pagesToExport {
            ...
            let image = renderer.image { context in
                // Check if this page has a PDF background (first pass)
                var hasPDFBackground = false
                ...
                // Fill background and draw paper pattern only for non-PDF pages
                if !hasPDFBackground {
                    UIColor(PaperUtilities.getPaperBackgroundColor(for: noteToExport.paperColor)).setFill()
                    UIRectFill(bounds)
                }
                
                // Draw background images/PDF
                if let imageDataArray = noteToExport.imageDataByPage[pageKey] {
                    for data in imageDataArray {
                ...
                // Paper pattern only for non-PDF pages
                if !hasPDFBackground {
    ...
    // Uses UIGraphicsBeginPDFContextToData to support variable page sizes.
    func exportNoteAsPDF(_ note: Note, selectedPages: [Int]? = nil) -> URL? {
        ...
        // Use NSMutableData with UIGraphicsBeginPDFContextToData for variable page sizes
        let pdfData = NSMutableData()
        // Pass .zero for bounds to allow per-page sizing
        ...
        for page in pagesToExport {
            ...
            if let pdfBackground = pdfBackground {
                ...
                // Draw the PDF page directly (no extra background fill or paper pattern for PDF pages)
                ...
                // Skip paper pattern for PDF backgrounds - they have their own content
