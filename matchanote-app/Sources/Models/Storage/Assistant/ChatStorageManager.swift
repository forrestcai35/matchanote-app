...
class ChatStorageManager: ObservableObject {
    ...
    func createNewConversation(noteId: UUID? = nil) {
        ...
        // Don't add to conversations array or save until messages are added
        print("✨ Created new conversation (not saved yet): \(newConversation.id)")
    }
    ...
    func updateCurrentConversation(with messages: [ChatMessage]) {
        ...
        if !messages.isEmpty {
            ...
            if let index = conversations.firstIndex(where: { $0.id == conversation.id }) {
                ...
                // First time adding this conversation (it now has messages)
            ...
            // Empty conversation - just update current without saving
            currentConversation = conversation
        }
    }
