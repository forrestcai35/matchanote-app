...
struct StorageNote: Codable {
  ...
  init(from decoder: Decoder) throws {
    ...
    // Handle backwards compatibility - default to empty dict if drawingDataByPage doesn't exist
...
class StorageManager: ObservableObject {
  ...
  // MARK: - Safe File Writing

  /// Atomically writes data to file with automatic backup for crash recovery
  /// Creates .backup file before overwriting, uses Swift's native atomic write
  private func writeDataAtomically(_ data: Data, to fileURL: URL) throws {
    let fileManager = FileManager.default
    let backupURL = fileURL.appendingPathExtension("backup")

    // Backup existing file if it exists (for crash recovery)
    ...
    // Use Swift's native atomic write - it handles temp file creation and replacement
    try data.write(to: fileURL, options: .atomic)
  }

  /// Attempts to load data from file, falls back to backup if main file is corrupted
  private func loadDataWithRecovery(from fileURL: URL) throws -> Data {
    let fileManager = FileManager.default
    let backupURL = fileURL.appendingPathExtension("backup")
    ...
    do {
      ...
      print("Error loading from main file: \(error). Attempting backup recovery...")
      ...
      if fileManager.fileExists(atPath: backupURL.path) {
        ...
        try writeDataAtomically(backupData, to: fileURL)
        print("Successfully recovered from backup: \(fileURL.lastPathComponent)")

        return backupData
      } else {
        // No backup available, re-throw original error
        throw error
      }
    }
  }
  ...
  func saveNoteSync(_ note: Note, completion: @escaping (Note) -> Void) {
    ...
    do {
      ...
      try self.writeDataAtomically(data, to: notesURL)
  ...
  func saveSubject(_ subject: Subject) -> Subject {
    ...
    Task.detached(priority: .utility) { [weak self] in
      ...
      do {
        let encoder = JSONEncoder()
        let data = try encoder.encode(self.subjects)
        try self.writeDataAtomically(data, to: subjectsURL)
      } catch {
        await MainActor.run {
  ...
  private func loadNotesFromLocal() {
    ...
    do {
      // Try loading with backup recovery
      let data = try loadDataWithRecovery(from: notesURL)
      let decoder = JSONDecoder()
      let storageNotes = try decoder.decode([StorageNote].self, from: data)
      self.notes = storageNotes.map { $0.toNote() }
    } catch {
      print("Error loading notes (backup recovery also failed): \(error)")
  ...
  private func loadFoldersFromLocal() {
    ...
    do {
      // Try loading with backup recovery
      let data = try loadDataWithRecovery(from: foldersURL)
      let decoder = JSONDecoder()
      ...
      print("Error loading folders (backup recovery also failed): \(error)")
  ...
  private func loadSubjectsFromLocal() {
    ...
    do {
      // Try loading with backup recovery
      let data = try loadDataWithRecovery(from: subjectsURL)
      let decoder = JSONDecoder()
      self.subjects = try decoder.decode([Subject].self, from: data)
    } catch {
      print("Error loading subjects (backup recovery also failed): \(error)")
  ...
  private func saveNoteLocally(_ note: Note) {
    ...
    Task.detached(priority: .utility) { [weak self] in
      ...
      do {
        let encoder = JSONEncoder()
        let data = try encoder.encode(notesToSave)
        // Use atomic write with backup for crash protection
        try self.writeDataAtomically(data, to: notesURL)
      } catch {
        await MainActor.run {
  ...
  private func saveFolderLocally(_ folder: Folder) {
    ...
    Task.detached(priority: .utility) { [weak self] in
      ...
      do {
        ...
        // Use atomic write with backup for crash protection
        try self.writeDataAtomically(data, to: foldersURL)
      } catch {
        await MainActor.run {
  ...
  private func updateStorageFiles() {
    ...
    Task.detached(priority: .utility) { [weak self] in
      ...
      do {
        ...
        // Update notes file with atomic write
        let notesData = try encoder.encode(notesStorageModels)
        try self.writeDataAtomically(notesData, to: notesURL)

        // Update folders file with atomic write
        let foldersData = try encoder.encode(foldersStorageModels)
        try self.writeDataAtomically(foldersData, to: foldersURL)
      } catch {
        await MainActor.run {
  ...
  private func updateStorageFilesBulk() {
    ...
    Task.detached(priority: .utility) { [weak self] in
      ...
      do {
        ...
        // Encode and write both files with atomic writes
        let notesData = try encoder.encode(notesStorageModels)
        let foldersData = try encoder.encode(foldersStorageModels)
