...
public class TextBoxManager: ObservableObject {
    ...
    public func addTextBox(
        to pageIndex: Int,
        at position: CGPoint = CGPoint(x: 100, y: 100),
        ...
        if text.isEmpty {
            ...
            boxSize = CGSize(width: 200, height: 30)
        } else {
            // Calculate size to fit the text content
            boxSize = calculateTextBoxSize(for: text, fontSize: fontSize, maxWidth: 400)
        }
    ...
    // Note: This is used for initial creation only - manual resizing allows text wrapping
    private func calculateTextBoxSize(for text: String, fontSize: CGFloat, maxWidth: CGFloat) -> CGSize {
        ...
        // Add padding to match UI (4pt on each side = 8pt total)
        let width = min(max(boundingRect.width + padding, 20 + padding), maxWidth)
