...
struct ResponsiveQuizHeader: View {
    ...
    private var compactLayout: some View {
        HStack(spacing: 8) {
            ...
            IconOnlyButton(icon: "plus.circle", color: .matchadark_light, action: onAddMore)
            IconOnlyButton(icon: "arrow.clockwise", color: .orange, action: onRegenerate)
        }
    }
            ...
            if let actionTitle = actionTitle, let action = action {
                Button(action: action) {
                    Text(actionTitle)
                        ...
                        .padding(.vertical, 10)
                        .background(Color.matchadark_light)
                        .clipShape(Capsule())
                }
                .buttonStyle(PlainButtonStyle())
            }
