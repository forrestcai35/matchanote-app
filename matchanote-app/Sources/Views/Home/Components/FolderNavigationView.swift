...
struct FolderNavigationView: View {
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 8)
                    .strokeBorder(
                        ...
                        lineWidth: isRootTargeted ? 2 : 0
                    )
            )
            .cornerRadius(8)
        }
        .buttonStyle(PlainButtonStyle())
        .onDrop(of: ["public.text"], isTargeted: $isRootTargeted) { _, _ in
    ...
    private func folderPathLink(for index: Int) -> some View {
        let isTargeted = targetedFolderIndex == index
        ...
        return Button(action: {
            ...
            .background(
                isTargeted
                    ? (colorScheme == .dark
                        ? Color.matchalight_dark.opacity(0.4)
                        : Color.matchalight_light.opacity(0.4))
        ...
        .onDrop(of: ["public.text"], isTargeted: Binding(
            get: { targetedFolderIndex == index },
            set: { newValue in
                targetedFolderIndex = newValue ? index : nil
            }
        )) { _, _ in
