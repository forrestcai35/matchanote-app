...
public struct GridFolderItemView: View {
    ...
    init(folder: Folder, isSelected: Bool = false, isSelectionMode: Bool = false, isDropTarget: Bool = false) {
        self.folder = folder
        ...
        VStack(spacing: 2) {
            ...
            ZStack {
                ...
                Image("folder")
                    ...
                    .shadow(
                        color: isDropTarget
                            ? (colorScheme == .dark ? Color.matchalight_dark.opacity(0.5) : Color.matchalight_light.opacity(0.5))
                            : Color.clear,
            ...
            .popover(isPresented: $showRenamePopover) {
                RenameFolderPopover(
                    ...
                    onSave: { newName in
                        _ = storageManager.updateFolderName(folderId: folder.id, newName: newName)
                    }
                )
            }
            .padding(.top, 5)
