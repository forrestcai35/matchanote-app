...
struct NoteView: View {
  ...
  // Returns the saved note with latest canvas data so callers can use it
  // immediately without waiting for the async storage update.
  @discardableResult
  private func saveCurrentCanvasData() -> Note? {
    ...
    GeometryReader { geometry in
      ZStack {
        VStack(spacing: 0) {
          ...
          HStack(spacing: 0) {
            if assistantOrientation == .left && isAssistantVisible {
              assistantPanelView()
            }
            ...
            if assistantOrientation == .right && isAssistantVisible {
              assistantPanelView()
            }
          }
        }
      ...
      .onDisappear {
        ...
        // Clear undo history to prevent glitched state when returning
        // This fixes the issue where the shared UndoManager retains actions for deallocated canvases
        for canvas in canvasManager.canvasViews {
          canvas.undoManager?.removeAllActions()
        }
        textBoxManager.undoManager?.removeAllActions()
        canvasManager.imageManager.undoManager?.removeAllActions()
...
-> some View
  ...
  private func assistantPanelView() -> some View {
    AIAssistantView(assistantOrientation: assistantOrientation)
      ...
      .onChange(of: tabManager.getActiveTab()?.note.id) { _, _ in
        if let activeTab = tabManager.getActiveTab() {
          // Update current note when switching tabs
          assistantState.currentNote = activeTab.note
          print("🔄 Note switched: AI Assistant now using '\(activeTab.note.title)'")
        }
      }
      // Drag gesture with high minimum distance to allow button taps
      .gesture(
        DragGesture(minimumDistance: 40, coordinateSpace: .local)
          .onChanged { value in
            // Only activate drag if starting from top 60 points (header area)
          ...
          .onEnded { value in
            // Only process drag end if it started in header area
            if value.startLocation.y <= 60 {
...
extension NoteView {
  ...
  private func handleQuickExport() {
    ...
    guard let latestNote = saveCurrentCanvasData() else {
