...
struct PageOverviewView: View {
    ...
    NavigationView {
      VStack(alignment: .leading, spacing: 0) {
        ...
        ZStack {
          ...
          // Left and right buttons overlaid
          HStack {
            // Left button (Done/Cancel)
            if isSelectionMode {
              Button("Cancel") {
                exitSelectionMode()
              }
            ...
            HStack(spacing: 12) {
              if isSelectionMode {
                ...
                // Page number display/editor
                HStack(spacing: 4) {
                  TextField("\(currentPage + 1)", value: Binding(
                    get: { currentPage + 1 },
                    set: { newValue in
                      let clampedValue = max(1, min(newValue, totalPages))
                      if clampedValue != currentPage + 1 {
                        currentPage = clampedValue - 1
                      }
                    }
                  ), format: .number)
  ...
  private func deleteSelectedPages() {
    ...
    // Calculate new page index locally first
    let remainingPageCount = totalPages - selectedPagesArray.count
    let newPageIndex: Int

    if selectedPages.contains(currentPage) {
      // Current page was deleted, move to the new last page (better UX than jumping to page 0)
      newPageIndex = max(0, remainingPageCount - 1)
    } else {
      ...
      newPageIndex = max(0, currentPage - deletedBefore)
    }

    // Update the note's current page BEFORE saving
    // This prevents the parent view from saving the old note state when the binding updates
    adjustedNote.currentPage = newPageIndex
    adjustedNote.dateModified = Date()
    
    let savedNote = storageManager.saveNote(adjustedNote)
    tabManager.updateNote(savedNote)

    // Delay binding update to ensure parent view receives the updated note from TabManager
    // before processing the page change (which triggers a save in NoteView).
    // This matches the behavior in PageManagement.swift to prevent race conditions.
