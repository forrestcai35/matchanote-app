...
class ToolState: ObservableObject {
  ...
  private enum DefaultsKeys {
    ...
    static let defaultTextBoxFontSize = "tool.defaultTextBoxFontSize"
    static let defaultTextBoxFontFamily = "tool.defaultTextBoxFontFamily"
    static let defaultTextBoxTextColor = "tool.defaultTextBoxTextColor"
    static let defaultTextBoxTextAlignment = "tool.defaultTextBoxTextAlignment"
  }
  ...
  @Published var defaultTextBoxFontSize: CGFloat = 12 {
    didSet {
      guard !isLoadingFromDefaults else { return }
      UserDefaults.standard.set(Double(defaultTextBoxFontSize), forKey: DefaultsKeys.defaultTextBoxFontSize)
    }
  }
  @Published var defaultTextBoxFontFamily: String = "System" {
    didSet {
      guard !isLoadingFromDefaults else { return }
      UserDefaults.standard.set(defaultTextBoxFontFamily, forKey: DefaultsKeys.defaultTextBoxFontFamily)
    }
  }
  @Published var defaultTextBoxTextColor: Color = .black {
    didSet { saveDefaultTextBoxTextColor() }
  }
  @Published var defaultTextBoxTextAlignment: TextAlignment = .leading {
    didSet {
      guard !isLoadingFromDefaults else { return }
      UserDefaults.standard.set(defaultTextBoxTextAlignment.rawValue, forKey: DefaultsKeys.defaultTextBoxTextAlignment)
    }
  }
  ...
  private func loadFromDefaults() {
    ...
    // Load default textbox properties
    if let fontSize = defaults.object(forKey: DefaultsKeys.defaultTextBoxFontSize) as? Double {
      defaultTextBoxFontSize = CGFloat(fontSize)
    }
    if let fontFamily = defaults.string(forKey: DefaultsKeys.defaultTextBoxFontFamily) {
      defaultTextBoxFontFamily = fontFamily
    }
    if let loadedColor = ColorPersistence.loadColor(forKey: DefaultsKeys.defaultTextBoxTextColor) {
      defaultTextBoxTextColor = loadedColor
    }
    if let alignmentRaw = defaults.string(forKey: DefaultsKeys.defaultTextBoxTextAlignment),
       let alignment = TextAlignment(rawValue: alignmentRaw) {
      defaultTextBoxTextAlignment = alignment
    }
  }
  ...
  private func saveDefaultTextBoxTextColor() {
    guard !isLoadingFromDefaults else { return }
    ColorPersistence.saveColor(defaultTextBoxTextColor, forKey: DefaultsKeys.defaultTextBoxTextColor)
  }
}
...
struct WrittenNoteToolbar: View {
  ...
  var body: some View {
    ...
    .onChange(of: canvasViews.count) { _, _ in
      // When canvas views change (e.g., switching tabs loads new canvases),
      // reapply the current tool to all canvases
      updateCanvasTool()
    }
    .zIndex(
  ...
  private func toolOptionsPanel(for tool: PenTool) -> some View {
                ...
                Button {
                  ...
                  ZStack {
                    ...
                    Circle()
                      ...
                      .frame(
                        width: dotDiameter(for: i, totalPresets: toolState.penWidthPresets.count),
                        height: dotDiameter(for: i, totalPresets: toolState.penWidthPresets.count)
                      )
                  }
                }
                .buttonStyle(PlainButtonStyle())
                ...
                Button {
                  ...
                  ZStack {
                    ...
                    Circle()
                      ...
                      .frame(
                        width: highlighterDotDiameter(for: i, totalPresets: toolState.markerWidthPresets.count),
                        height: highlighterDotDiameter(for: i, totalPresets: toolState.markerWidthPresets.count)
                      )
                  }
                }
                .buttonStyle(PlainButtonStyle())
                ...
                Circle()
                  ...
                  .frame(
                    width: toolState.eraserAreaDotSizes[safe: i]
                      ?? dotDiameter(for: i, totalPresets: toolState.eraserAreaWidthPresets.count),
    ...
    case .textbox:
      HStack(spacing: 12) {
        // Always show formatting controls (for both default settings and editing)
        TextBoxFormattingControls(textBoxManager: textBoxManager, toolState: toolState)
      }
      .frame(maxWidth: .infinity, alignment: .leading)
    case .shape:
      HStack(spacing: 12) {
  ...
  // Map actual width value to a visually distinct diameter for the preset dot
  private func dotDiameter(for index: Int, totalPresets: Int) -> CGFloat {
    let minDiam: CGFloat = 4
    let maxDiam: CGFloat = 24
    // Use actual width value to calculate dot size (pen practical range is 0.5...6)
    let width = toolState.penWidthPresets[safe: index] ?? 3.0
    let fraction = min((width - 0.5) / (6.0 - 0.5), 1.0)
    return minDiam + fraction * (maxDiam - minDiam)
  }

  // Map actual width value to a visually distinct diameter for the highlighter preset dot
  private func highlighterDotDiameter(for index: Int, totalPresets: Int) -> CGFloat {
    let minDiam: CGFloat = 8
    let maxDiam: CGFloat = 26
    // Use actual width value to calculate dot size (marker range is 4...20)
    let width = toolState.markerWidthPresets[safe: index] ?? 12.0
    let fraction = (width - 4.0) / (20.0 - 4.0)
    return minDiam + fraction * (maxDiam - minDiam)
  }
